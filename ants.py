    ...
    def remove_insect(self, insect):
        ...
        if type(insect) is QueenAnt:
            ...
            if type(insect) == BodyguardAnt:
                ...
                self.ant, insect.place = None, None
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        ranges, place = 0, self.place
        while place and place != hive:
            if place.bees and self.min_range <= ranges <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 1:
            ...
            for x in range(len(copy_bees)):
                copy_bees[x].reduce_armor(self.damage) 
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        for x in range(len(copy)):
            copy[x].reduce_armor(self.damage)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        """
        "*** YOUR CODE HERE ***"
        def entrance_helper(place):
            if not place.entrance:
                return
            if place.ant and type(place.ant) != QueenAnt:
                if place.ant not in QueenAnt.doubledA:
                    if type(place.ant) == BodyguardAnt and type(place.ant.ant) != QueenAnt:
                        place.ant.ant.damage *= 2
                        QueenAnt.doubledA += [place.ant]
                    place.ant.damage *= 2
                    QueenAnt.doubledA += [place.ant]
            return entrance_helper(place.entrance)
        def exit_helper(place):
            if not place.exit:
                return
            if place.ant and type(place.ant) != QueenAnt:
                if place.ant not in QueenAnt.doubledA:
                    ...
                    place.ant.damage *= 2
                    QueenAnt.doubledA += [place.ant]
            return exit_helper(place.exit)
        
        if self.name == 'Impostor':
            self.reduce_armor(self.armor)   
        else:
            exit_helper(self.place)
            entrance_helper(self.place)
            self.throw_at(self.nearest_bee(colony.hive))
